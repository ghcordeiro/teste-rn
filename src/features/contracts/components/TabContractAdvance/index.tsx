import CardDetailsAdvance from '@components/CardDetailsAdvance';
...
import { IAdvance } from '../../types/contract';

interface ITabContractAdvanceProps {
  id: string;
}

const TabContractAdvanceNF = ({ id }: ITabContractAdvanceProps) => {
  const listRef = useRef(null);

  const [limit] = useState(20);
  const [allRows, setAllRows] = useState(-1);
  const [data, setData] = useState<Array<IAdvance>>([] as Array<IAdvance>);
  const [loadingNF, setLoadingNF] = useState(false);

  const renderItem = (item: IAdvance, index: number) => (
    <View style={{ paddingTop: 8 }}>
      <CardDetailsAdvance data={item} index={index} contractId={id} />
  ...
  const handleLoad = useCallback(async () => {
    try {
      ...
      const response = await api.get(
        `salescontract/get/${id}/advance-invoice?skip=${data.length}&limit=${limit}`,
      );
...
export default TabContractAdvanceNF;
