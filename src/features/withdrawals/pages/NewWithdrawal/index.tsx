...
const Withdrawal = () => {
  ...
  const [data, setData] = useState<Array<IWithdrawal>>(
    [] as Array<IWithdrawal>,
  );
      ...
      try {
        ...
        try {
          const response = await api.get(
            `orderstock/withdrawal-balance?skip=${
              currentData.length
          ...
          console.error('Request orderstock/withdrawal-balance.Error: ', error);
        }
      } catch (e) {
        console.log(e);
      } finally {
  ...
  const renderItem = (item: IWithdrawal, index: number) => (
    <>
      <Container>
        <CardWithdrawal
          key={`withdrawal-${item.productId}-${index}`}
          data={item}
        />
        {index === data.length - 1 ? (
