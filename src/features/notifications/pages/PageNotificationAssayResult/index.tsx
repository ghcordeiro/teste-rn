...
import { IMobileNotification } from '../../types/notification';
...
const PageNotificationAssayResult = () => {
  ...
  const loadRepositories = useCallback(async () => {
    try {
      ...
      const response = await api.get(
        `assayresult/list?skip=${data.length}&limit=${limit}&sort=-createdAt`,
      );
  ...
  const renderItem = (item: IMobileNotification, index: number) => {
    item.type = EFirebaseNotificationType.ASSAYRESULT;
    item.notificationOriginId = item._id;
    return (
      <CardNotification
        key={`${item._id}-${index}`}
        data={item}
...
export default PageNotificationAssayResult;
