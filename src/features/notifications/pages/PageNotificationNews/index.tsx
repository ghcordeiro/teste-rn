...
const PageNotificationNews = () => {
  ...
  const loadRepositories = useCallback(async () => {
    try {
      ...
      const response = await api.get(
        `news/list?skip=${data.length}&limit=${limit}&sort=-createdAt`,
      );
  ...
  const renderItem = (item: IMobileNotification, index: number) => {
    item.type = EFirebaseNotificationType.NEWS;
...
export default PageNotificationNews;
