...
import { IPayment, IPaymentItem } from 'src/features/contracts';
...
export interface IModalCardDetailsPaymentProps {
  openModal: () => void;
  closeModal: () => void;
}
...
const ModalCardDetailsPayment: React.ForwardRefRenderFunction<
  IModalCardDetailsPaymentProps,
  ReciveProps
> = ({ payment, contractId }, ref) => {
  const [visible, setVisible] = useState<boolean>(false);
  const [loading, setLoading] = useState<boolean>(false);
  const [data, setData] = useState<IPaymentItem>({} as IPaymentItem);
  ...
  const loadRepositories = async () => {
    try {
      ...
      const response = await api.get(
        `salescontract/get/${contractId}/payment-invoice/${payment._id}`,
      );
...
export default forwardRef(ModalCardDetailsPayment);
