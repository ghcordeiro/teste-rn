...
import CardFinancialMovimentModalCashFlow from '@components/CardFinancialMovimentModalCashFlow';
...
> = ({chave, filters}, ref) => {
  ...
  const getFiltersQuery = (filters?: ICashFlowFilters): string => {
    ...
    if (filters.status && filters.status.length > 0) queryParams.push(`status=${filters.status}`);
    if (filters.documentType) queryParams.push(`documentType=${filters.documentType}`);
    if (filters.producer) queryParams.push(`producer=${filters.producer || 'ALL'}`);
    if (filters.expirationdateend) queryParams.push(`expirationdateend=${filters.expirationdateend}`);
    if (filters.expirationdatenextperiod) queryParams.push(`expirationdatenextperiod=${filters.expirationdatenextperiod}`);
  ...
  const listEmptyComponent = () => (
    <CenteredFlex>
      <TextRegular size={16}>{translate('noItems')}</TextRegular>
    </CenteredFlex>
  );
