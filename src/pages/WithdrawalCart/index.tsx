...
import { CardWithdrawalCart } from '@components/CardWithdrawalCart';
...
export function WithdrawalCart() {
  ...
  const loadOptions = async () => {
    ...
    try {
      ...
      const farms = ((response.data || []) as Array<IOptionAddress>).map( doc => ({...doc, farmNameAddress: `${doc.farmName ? `${doc.farmName} - `: ''}${doc.farmAddress}`}))
      if (farms && farms.length > 1){
        farms.unshift({ farmName: "", farmAddress: "", farmNameAddress: "Selecione um Endereço para Faturamento", })
      }
  ...
  const handleClear = () => {
    Alert.alert(
      ...
      [{ text: 'Cancelar' }, { text: 'Limpar', onPress: (a) => {
        clearItems()
  ...
  const renderItem = (item: ISaveWithdrawalDto, index: number) => (
    <>
      <Container>
        <CardWithdrawalCart
          key={item.product.id.toString() + String(Date.now() * Math.random())}
          data={item}
          index={index}
        />
  ...
  const handleSaveWithdrawal = async () => {
    if (items.length > 0) {
      ...
      Alert.alert('Ao menos um item deve ser selecionado!');
    }
  };
  ...
  const handleConfirmWithdrawal = async () => {
    try {
      ...
      if (options && options?.length > 0 && !address){
        Alert.alert('Informe o Endereço para Faturamento!');
        setSending(false)
        return;
      }
      if (!vehiclePlate){
        Alert.alert('Informe a Placa do Veículo para Retirada!');
        setSending(false)
        return;
      }
      const plateRegex = new RegExp('^[A-Z]{3}[0-9][A-Z][0-9]{2}$|^[A-Z]{3}[0-9]{4}');
      if (!plateRegex.test(vehiclePlate.toUpperCase())) {
        Alert.alert('Placa do veículo inválida!', 'A placa deve estar no formato AAA0000 ou AAA0A00.');
        setSending(false);
        return;
      }
      ...
      try {
        ...
        Alert.alert('Solicitação de retirada salva com sucesso!');
        clearItems();
        setAddress(undefined)
        setVehiclePlate('');
        ...
        console.error(e);
        Alert.alert('Ops! Erro inesperado ao finalizar retirada. \nErro: '+e);
      }
    } catch (error) {
  ...
  const maskPlate = (value?: string) => {
    ...
    if (/^[A-Z]{3}\d[A-Z]\d{0,2}$/.test(s) || /^[A-Z]{3}\d[A-Z]\d{2}$/.test(s)) {
      return s;
    }

    // Caso “antigo” (LLL DDDD) – insere hífen após as 3 letras
    return s.replace(/^([A-Z]{3})(\d{0,4}).*$/, (_, l3, rest) => (rest ? `${l3}${rest}` : l3));
  };
