...
import { ICashFlow, ICashFlowResume } from '@dtos/cash-flow';
...
import FilterModalCashFlow, { IOutCashFlowFilters } from './FilterModalCashFlow';
...
const CashFlow = () => {
  ...
  const [operationFilter, setOperationFilter] = useState<EFinancialStatementOperation>();
  const [statusFilter, setStatusFilter] = useState<string[]>();
  const [documentTypeFilter, setDocumentTypeFilter] = useState<string>();
  const [producerFilter, setProducerFilter] = useState<string>('ALL');
  const [expirationDateEndFilter, setExpirationDateEndFilter] = useState<number>();
  const [expirationDateNextPeriodFilter, setExpirationDateNextPeriodFilter] = useState<number>();

  const [cashFlowData, setCashFlowData] = useState<Record<string, ICashFlowResume[]>>({ 'R$': [], 'US$': [] });
  const [allRowsMap, setAllRowsMap] = useState<Record<string, number>>({ 'R$': -1, 'US$': -1 });
  const [loadingTabs, setLoadingTabs] = useState<Record<string, boolean>>({ 'R$': false, 'US$': false });

  const getFilters = (filters: ICashFlowFilters): string[] => {
    const { status, producer, documentType, expirationdateend, expirationdatenextperiod, operation } = filters;
    const filter: string[] = [];
    ...
    if (expirationdateend) filter.push(`expirationdateend=${expirationdateend}`)
    if (expirationdatenextperiod) filter.push(`expirationdatenextperiod=${expirationdatenextperiod}`)
    return filter;
  };
  ...
  const handleLoadGeneral = useCallback(async (filters: ICashFlowFilters) => {
    ...
    try {
      const response = await api.get(`financialmovement/cash-flow/balance?${query.join('&')}`);
  ...
  const loadTabData = async (currency: string, filters: ICashFlowFilters) => {
    ...
    try {
      ...
      const response = await api.get(`financialmovement/cash-flow/position/${currency}?${query.join('&')}`);
  ...
  const onRefreshData = useCallback(async (filters: ICashFlowFilters) => {
  ...
  const handleFilter = useCallback(async (filters?: ICashFlowFilters) => {
    await onRefreshData({
  ...
  return (
    <>
            ...
            {loadingProgress ? <Loading /> : (
              <>
                ...
                <Row
                  ...
                  <Flex>
                    <TextBold
                      size={11}>{`SALDO (${dataGeneral.currency})`}</TextBold>
                    <TextBold
                      size={11}>{`PTAX (${dataGeneral.ptax})`}</TextBold>
                  </Flex>
                  <Flex>
                    <TextRegular size={11}>
                      {convertCurrency(
      ...
      <FloatingButton filterCount={modalRef.current?.filterCount || 0} open={modalRef.current?.openModal} />
    </>
  );
};
