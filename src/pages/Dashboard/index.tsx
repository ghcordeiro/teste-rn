...
const Dashboard = () => {
  ...
  const [_loadingTopVendas, setLoadingTopVendas] = useState<boolean>(true);
  ...
  const loadTopVendas = useCallback(
    async ({ crop, culture }: IDashFilterProps) => {
      try {
  ...
  const loadSalesPosition = useCallback(
    async ({ crop, culture }: IDashFilterProps) => {
      try {
  ...
  const loadCardGraph = useCallback(
    async ({ crop, culture }: IDashFilterProps) => {
      try {
  ...
  const handleSetCropCultureFilter = useCallback(
    async (cropCulture: string) => {
        ...
        {!onLoadingCrop && (
          <>
            ...
            <StyledView marginTop={24} marginBottom={36}>
              {producersList.map(r => (
                <ButtonSelect onPress={() => handleSelect(r.id)} key={r.id}>
